...
static void reschedule_delete_dist_entry(DistEntry* dep)
{
     ...
     *
     * Note that timeouts does not guarantee thread progress.
     */
    if (node_tab_delete_delay == 0) {
        erts_schedule_thr_prgr_later_op(prepare_try_delete_dist_entry,
                                        dep, &dep->later_op);
    }
    else {
...
setup_reference_table(void)
{
    ...
    erts_debug_callback_timer_foreach(prepare_try_delete_dist_entry,
				      ...
				      NULL);
    erts_debug_later_op_foreach(prepare_try_delete_dist_entry,
