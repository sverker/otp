...
#    define ERTS_HIGHEST_CALLER_SAVE_XREG 5
     ...
     */
    const Uint64 bad_boxed_ptr = 0xcafedead00002UL;
...
protected:
    ...
    void emit_enter_runtime(int live = 0) {
        ...
        if (Spec & Update::eFragileXregs) {
            ...
            /* Save register-backed X registers to the X register
             * array. The backing registers must NOT be used
             * afterwards. In a DEBUG build, the backing X registers
             * will be overwritten with garbage values. */
            int num_to_save = live;
            ...
            for (int i = 0; i < num_to_save; i += 2) {
                ...
                arm::Gp reg2 = register_backed_xregs[i + 1];
                if (i == 0) {
                    mov_imm(XREG0, bad_boxed_ptr + 0x80 + (Spec << 8));
                } else {
                    a.mov(reg1, XREG0);
                }
                if (i + 1 < num_to_save) {
         ...
         * unless they were already destroyed above.
         */
        if (live < num_register_backed_xregs) {
            mov_imm(XREG3, bad_boxed_ptr + 0x80 + (Spec << 8));
            a.mov(XREG4, XREG3);
            a.mov(XREG5, XREG3);
        }
#endif
    ...
    void emit_leave_runtime(int live = 0) {
        ...
        if (Spec & Update::eFragileXregs) {
             ...
             * array must not be used afterwards. In a DEBUG build, the
             * registers in the X register array will be overwritten with
             * garbage values.
             */
            int num_to_load = num_register_backed_xregs;
...
#ifdef DEBUG
            mov_imm(SUPER_TMP, bad_boxed_ptr + 0x20);
            for (int i = 0; i < num_to_load; i += 2) {
                arm::Gp reg1 = register_backed_xregs[i];
                arm::Gp reg2 = register_backed_xregs[i + 1];
                if (i + 1 == num_to_load) {
...
#include "beamasm_protos.h"
    ...
    void copy_reg(arm::Gp to, const JitRegister<arm::Gp> &from) {
        if (to != from.reg) {
            a.mov(to, from.reg);
        }
    }
