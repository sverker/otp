...
void BeamGlobalAssembler::emit_check_float_error() {
    const Uint double_max = 0x7FEFFFFFFFFFFFFFul;
    Label error = a.newLabel();

    a.fabs(a64::d30, a64::d0);
    mov_imm(TMP1, double_max);
    a.fmov(a64::d31, TMP1);
...
void BeamModuleAssembler::emit_float_instr(uint32_t instId,
                                           ...
                                           const ArgVal &RHS,
                                           const ArgVal &Dst) {
    JitRegister<arm::VecD> lhs = init_src_reg(LHS, a64::d0);
    JitRegister<arm::VecD> rhs = init_src_reg(RHS, a64::d1);
    JitRegister<arm::VecD> dst = init_dst_reg(Dst, a64::d2);
...
void BeamModuleAssembler::emit_fload(const ArgVal &Src, const ArgVal &Dst) {
    JitRegister<arm::Gp> src = init_src_reg(Src, TMP1);
    JitRegister<arm::VecD> dst = init_dst_reg(Dst, a64::d0);
...
void BeamModuleAssembler::emit_fstore(const ArgVal &Src, const ArgVal &Dst) {
    JitRegister<arm::VecD> src = init_src_reg(Src, a64::d0);
    JitRegister<arm::Gp> dst = init_dst_reg(Dst, TMP1);
...
void BeamModuleAssembler::emit_i_fnegate(const ArgVal &Src, const ArgVal &Dst) {
    JitRegister<arm::VecD> src = init_src_reg(Src, a64::d0);
    JitRegister<arm::VecD> dst = init_dst_reg(Dst, a64::d1);
