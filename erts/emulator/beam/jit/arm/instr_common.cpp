...
void BeamModuleAssembler::emit_gc_test_preserve(const ArgVal &Need,
    ...
    if (Live.getValue() < num_register_backed_xregs) {
        a.mov(register_backed_xregs[Live.getValue()], term);
    } else {
        a.str(term, getXRef(Live.getValue()));
    }
    ...
    if (Live.getValue() < num_register_backed_xregs) {
        a.mov(term, register_backed_xregs[Live.getValue()]);
    } else {
        a.ldr(term, getXRef(Live.getValue()));
    }
...
void BeamModuleAssembler::emit_is_function2(const ArgVal &Fail,
    ...
    emit_branch_if_ne(TMP2, arity, fail);
    // a.cmp(TMP2, imm(arity));
    // a.cond_ne().b(fail);
    a.b(next);
