...
static ERL_NIF_TERM enabled(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if (enif_compare(argv[1], argv[2]) == 0) {
        return atom_discard;
    }
...
static ERL_NIF_TERM trace(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if (enif_compare(argv[4], atom_undefined) != 0) {
        tt[tt_sz++] = atom_trace;
        tt[tt_sz++] = argv[2];
        tt[tt_sz++] = argv[0];
        tt[tt_sz++] = argv[3];
        ...
        if (enif_compare(argv[0], atom_seq_trace) == 0) {
    ...
    if (enif_get_map_value(env, opts, atom_match_spec_result,
                           &value)
        && enif_compare(value, atom_true) != 0) {
        tt[tt_sz++] = value;
    }

    if (enif_get_map_value(env, opts, atom_scheduler_id, &value)
        && enif_compare(value, atom_undefined) != 0) {
        tt[tt_sz++] = value;
    }
    ...
    if (enif_get_map_value(env, opts, atom_timestamp, &value)
        && enif_compare(value, atom_undefined) != 0) {
        ERL_NIF_TERM ts;
        if (enif_compare(value, atom_monotonic) == 0) {
        ...
        } else if (enif_compare(value, atom_cpu_timestamp) == 0) {
