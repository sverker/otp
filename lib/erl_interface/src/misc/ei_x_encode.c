...
{
    return ei_x_encode_atom_len2(x, s, strlen(s), ERLANG_LATIN1, ERLANG_LATIN1);
}

int ei_x_encode_atom_len(ei_x_buff* x, const char* s, int len)
{
    return ei_x_encode_atom_len2(x, s, len, ERLANG_LATIN1, ERLANG_LATIN1);
}

int ei_x_encode_atom2(ei_x_buff* x, const char* s,
		      enum erlang_char_encoding from_enc,
		      enum erlang_char_encoding to_enc)
{
    return ei_x_encode_atom_len2(x, s, strlen(s), from_enc, to_enc);
}

int ei_x_encode_atom_len2(ei_x_buff* x, const char* s, int len,
			  enum erlang_char_encoding from_enc,
			  enum erlang_char_encoding to_enc)
{
    int i = x->index;
    ei_encode_atom_len2(NULL, &i, s, len, from_enc, to_enc);
    if (!x_fix_buff(x, i))
	return -1;
    return ei_encode_atom_len2(x->buff, &x->index, s, len, from_enc, to_enc);
}
